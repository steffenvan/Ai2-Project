...
from extraction import abstract_txt_string
...
def preprocess_file_abs(input_filename, output_filename, english_dictionnary_name, corpus_dictionnary_name = "", min = 1) :
...
def augment_word_list(filename, word_list_name, en_dico_name) :
    ...
    doc = nlp(unicode(open(filename).read(), "utf-8"))
    for token in doc :
